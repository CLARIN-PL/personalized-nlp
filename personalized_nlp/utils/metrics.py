...
def evaluate_classification(test_predictions, true_labels, class_dims):
    ...
    for cls_idx in range(len(class_dims)):
        ...
        dims_results[cls_idx] = classification_report(
...
def evaluate_regression(test_predictions, true_labels):
    if true_labels.ndim > 1:
        losses = [r2_score(true_labels[:, i], test_predictions[:, i])
...
class PrecisionClass(torchmetrics.Precision):
    ...
    def compute(self) -> Tensor:
        ...
        return precision_for_classes[-1]
