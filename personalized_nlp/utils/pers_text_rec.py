...
def random_assignment(
        ...
        max_annotations_per_user: Optional[int] = None) -> pd.DataFrame:
    merged_annotations['user_annotation_order'] = np.random.permutation(len(merged_annotations.index))
    merged_annotations['user_annotation_order'] = merged_annotations.groupby('annotator_id')['user_annotation_order'].apply(lambda x: np.argsort(x))
