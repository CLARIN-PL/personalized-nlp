import torch
import torch.nn as nn

from personalized_active_learning.datamodules import TextFeaturesBatch
from personalized_active_learning.models.interface import IModel


class PersonalizedBaseline(IModel):
    def __init__(
        self,
        output_dim=2,
        text_embedding_dim=768,
        **kwargs,  # TODO: eliminate kwargs / We need it to define different models
    ):
        super().__init__()
        self.text_embedding_dim = text_embedding_dim
        # +1 since we provide annotator id as well
        self.fc1 = nn.Linear(text_embedding_dim + 1, output_dim)

    def get_head(self) -> nn.Module:
        """Get the model's head.

        Returns: The model's head (I.E. last layer).

        """
        return self.fc1

    def set_head(self, new_head: nn.Module):
        """Set the model's head.

        Args:
            new_head: The new model's head.

        """
        self.fc1 = new_head

    def forward(self, features: TextFeaturesBatch):
        x = torch.column_stack(
            [
                features.annotator_ids.view((-1, 1)),
                features.embeddings.view(-1, self.text_embedding_dim),
            ]
        )
        x = self.fc1(x)

        return x
